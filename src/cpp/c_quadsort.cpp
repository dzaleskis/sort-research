...
uint32_t indirect_sort_impl(T* data,size_t len) noexcept {
  try {
      ...
      quadsort(static_cast<void*>(ptrArray), len, sizeof(T*), indirect_c_cmp_func<T_CPP>);
...
uint32_t indirect_sort_by_impl(T* data,size_t len, CompResult (*cmp_fn)(const T&, const T&, uint8_t*), uint8_t* ctx) noexcept {
  try {
      ...
      quadsort(static_cast<void*>(ptrArray), len, sizeof(T*), make_indirect_compare_by_fn_c(cmp_fn, ctx));
