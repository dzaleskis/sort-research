...
sort_impl!("rust_max_mergesort_stable");
    ...
    F: FnMut(&T, &T) -> bool,
{
    ...
    while end > 0 {
        ...
        if runs.len() >= 1 {
            unsafe {
                let prev_run = runs.last().unwrap_unchecked();
                ...
                runs.last_mut().unwrap_unchecked().power = power;
            }
        }
    ...
    F: FnMut(&T, &T) -> bool,
{
    ...
    if current_run_len < FAST_SORT_SIZE && end >= FAST_SORT_SIZE && T::is_freeze() {
        ...
        sort32_stable(&mut v[start..end], buf, is_less);
    } else {
    ...
    F: FnMut(&T, &T) -> bool,
{
    ...
    if mid <= len - mid {
        ...
        // run the allowed amount of cross merges
        while (*left).add(1) < hole.end && right.add(1) < v_end {
            // Consume the lesser side.
            // If equal, prefer the left run to maintain stability.
            unsafe {
                if !is_less(&*right, &*(*left).add(1)) {
                    // optimal merge left
                    ptr::copy_nonoverlapping(*left, *out, 2);
                    *left = left.add(2);
                    *out = out.add(2);
                } else if is_less(&*right.add(1), &**left) {
                    // optimal merge right
                    ...
                    *out = out.add(2);
                    ...
                    ptr::copy_nonoverlapping(
                        ...
                        out.add(right_is_l as usize),
                        1,
                    );
                    ptr::copy_nonoverlapping(
                        get_and_increment(&mut right),
                        out.add(right_not_l as usize),
                        1,
                    );
                    *out = out.add(2);
                }
        ...
        while *left < hole.end && right < v_end {
            // Finish consuming the lesser side.
            // If equal, prefer the left run to maintain stability.
            unsafe {
            ...
            unsafe {
                if is_less(&*right.sub(1), &*left.sub(2)) {
                    ...
                    *right = right.sub(2);
                    ...
                    // branchless merge left/right
                    let right_is_l = is_less(&*right.offset(-1), &*left.offset(-1));
                    let right_not_l = !right_is_l;
...
#[cfg_attr(feature = "no_inline_sub_functions", inline(never))]
fn sort32_stable<T, F>(v: &mut [T], buf: *mut T, is_less: &mut F)
where
    F: FnMut(&T, &T) -> bool,
{
